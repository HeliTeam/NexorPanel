...
import (
	...
	"github.com/nexor/panel/web/session"
...
func (a *IndexController) login(c *gin.Context) {
	...
	if locked, retry := middleware.IsLoginLocked(getRemoteIp(c), form.Username); locked {
		pureJsonMsg(c, http.StatusOK, false, fmt.Sprintf("%s (%ds)", I18nWeb(c, "pages.login.toasts.loginLocked"), retry))
		return
	}

	user, checkErr := a.adminService.CheckAdmin(form.Username, form.Password, form.TwoFactorCode)
	...
	if user == nil {
		...
		notifyPass := safePass
		is2FA := checkErr != nil && checkErr.Error() == "invalid 2fa code"
		if is2FA {
			translatedError := a.tgbot.I18nBot("tgbot.messages.2faFailed")
			notifyPass = fmt.Sprintf("*** (%s)", translatedError)
		} else {
			if locked, retry := middleware.RecordLoginFailure(getRemoteIp(c), form.Username); locked {
	...
	middleware.ResetLoginFailures(getRemoteIp(c), form.Username)
...
func (a *IndexController) logout(c *gin.Context) {
	...
	if user != nil {
		logger.Infof("%s logged out successfully", user.Nickname)
	}
