// Package logger provides logging functionality for the Nexor panel with
...
const (
	...
	logFileName      = "nexor.log"           // Log file name
...
func InitLogger(level logging.Level) {
	newLogger := logging.MustGetLogger("nexor")
	...
	if consoleBackend := initDefaultBackend(); consoleBackend != nil {
		...
		leveledBackend.SetLevel(level, "nexor")
	...
	if fileBackend := initFileBackend(); fileBackend != nil {
		...
		leveledBackend.SetLevel(logging.DEBUG, "nexor")
		backends = append(backends, leveledBackend)
	}
