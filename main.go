// Package main is the entry point for the Nexor web panel application.
...
import (
	...
	"github.com/nexor/panel/sub"
	"github.com/nexor/panel/util/crypto"
	"github.com/nexor/panel/util/sys"
	"github.com/nexor/panel/web"
	"github.com/nexor/panel/web/global"
...
// runWebServer initializes and starts the web server for the Nexor panel.
func runWebServer() {
...
func showSetting(show bool) {
	if show {
		...
		adminModel, err := adminService.GetFirstAdmin()
		if err != nil {
			fmt.Println("get current admin info failed, error info:", err)
		}

		if adminModel == nil || adminModel.Nickname == "" || adminModel.PasswordHash == "" {
			fmt.Println("current nickname or password is empty (use create-admin to bootstrap)")
		}
		...
		hasDefaultCredential := func() bool {
			...
			return adminModel.Nickname == "admin" && crypto.CheckPasswordHash(adminModel.PasswordHash, "admin")
		}()
...
func updateSetting(port int, username string, password string, webBasePath string, listenIP string, resetTwoFactor bool) {
	...
	if username != "" || password != "" {
		err := adminService.UpdateFirstAdmin(username, password)
		if err != nil {
			fmt.Println("Failed to update nickname and password:", err)
		} else {
			fmt.Println("Nickname and password updated successfully")
		}
	}
...
// runCreateAdmin interactively creates a panel administrator (bcrypt-hashed password).
func runCreateAdmin() {
	err := database.InitDB(config.GetDBPath())
	if err != nil {
		fmt.Println("Database initialization failed:", err)
	...
	if err := svc.CreateAdmin(nickname, string(pw1)); err != nil {
...
// migrateDb performs database migration operations for the Nexor panel.
func migrateDb() {
...
// main is the entry point of the Nexor application.
...
func main() {
	...
	flag.Usage = func() {
		...
		fmt.Println("    create-admin   create panel administrator (interactive)")
	}
